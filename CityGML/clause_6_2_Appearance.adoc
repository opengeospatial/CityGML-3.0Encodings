...
The CityGML Appearance XML elements and their corresponding CityGML UML classes are shown in <<appearance-xml-elements>>. In addition, the CityGML Appearance XML elements are documented in the GML schema http://schemas.opengis.net/citygml/3.0/appearance.xsd[appearance.xsd] and in this link:XML-schema-documentation/appearance.xsd.html[HTML documentation].
